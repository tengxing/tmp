...
<script type="text/javascript">
	...
	function submitData(){
		var title=$("#title").val();
		var type=$("#blogTypeId").combobox("getValue");
		...
		if(title==null || title==''){
			...
			$.post("${pageContext.request.contextPath}/admin/new/save",{'title':title,'type':type,'content':content,'summary':UE.getEditor('editor').getContentTxt().substr(0,155)},function(result){
				if(result.success){
					alert("新闻发布成功！");
					resetValue();
				}else{
...
<div id="p" class="easyui-panel" title="写新闻" style="padding: 10px">
   		...
   		<tr>
   			...
   			<td>
   				<select class="easyui-combobox" style="width: 154px" id="blogTypeId" name="blogType.id" editable="false" panelHeight="auto" >
					...
					<option value="分行业资讯">行业资讯</option>
                </select>
   			</td>
   		</tr>
   		<tr>
