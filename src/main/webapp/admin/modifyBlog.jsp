...
<script type="text/javascript">
	...
	function submitData(){
		...
		if(title==null || title==''){
			...
			$.post("${pageContext.request.contextPath}/admin/new/save",{'id':'${param.id}','title':title,'type':blogTypeId,'content':content,'summary':UE.getEditor('editor').getContentTxt().substr(0,155)},function(result){
				if(result.success){
					alert("文章修改成功！");
				}else{
					alert("文章修改失败！");
...
<div id="p" class="easyui-panel" title="修改新闻" style="padding: 10px">
 ...
 <script type="text/javascript">
    ...
    ue.addListener("ready",function(){
        //通过ajax请求数据
        UE.ajax.request("${pageContext.request.contextPath}/admin/new/findById",
            {
                method:"post",
                async : false,  
                data:{"id":"${param.id}"},
                onsuccess:function(result){
                	...
                	$("#title").val(result.title);
       				$("#blogTypeId").combobox("setValue",result.type);
