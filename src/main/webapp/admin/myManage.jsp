...
<script type="text/javascript">
	...
	function deleteCourseType(){
		...
		if(selectedRows.length==0){
		 ...
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${pageContext.request.contextPath}/admin/sign/delete",{ids:ids},function(result){
						if(result.success){
							if(result.exist){
	...
	function openCourseTypeAddDialog(){
		...
		url="${pageContext.request.contextPath}/admin/sign/save";
	}
	...
	function openCourseTypeModifyDialog(){
		var selectedRows=$("#dg").datagrid("getSelections");
		 ...
		 $("#fm").form("load",row);
		 url="${pageContext.request.contextPath}/admin/sign/save?id="+row.id;
	 }
...
<table id="dg" title="登记用户管理" class="easyui-datagrid"
   ...
   url="${pageContext.request.contextPath}/admin/sign/list" fit="true" toolbar="#tb">
   <thead>
   	<tr>
   ...
   <form id="fm" method="post">
   	<table cellspacing="8px">
   		...
   		<tr>
           			<td>内容：</td>
           			<td>
           			<textarea name = "content" rows = 5 cols = 35 class="easyui-validatebox" required="true"/></textarea>
           			</td>
           		</tr>
   	</table>
   </form>
